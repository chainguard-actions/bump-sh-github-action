...
export class Repo {
    ...
    async getBaseOverlays(overlays) {
        const tmpDir = 'tmp/';
        const tmpOverlays = await Promise.all((overlays || []).map(async (overlayFile) => {
            if (!overlayFile)
                return undefined;
    ...
    async createOrUpdateComment(body, digest) {
        ...
        if (existingComment) {
            ...
            const existingDigest = extractBumpDigest(_docDigest, existingComment.body);
    ...
    async findExistingComment(issue_number) {
        ...
        return comments.data.find((comment) => extractBumpDigest(this._docDigest, comment.body || ''));
    }
